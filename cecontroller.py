...
class ceclient():
    ...
    def upload_file(self, projectname):
        ...
        if os.path.isfile(fullpath_projectname):
            ...
            for instance in all_instances_name:
                ...
                command_arr = []
                command_arr.append('call gcloud compute ssh ' + temp_instance + ' --zone ' + zone + ' --command "cd ' + temp_file_path + '; sudo rm -rf ' + temp_file_path + projectname.split('.')[0] + '; sudo rm -f ' + temp_file_path + projectname.split('.')[0] + '*" -q \n')
                command_arr.append('call gcloud compute scp "' + fullpath_projectname + '" ' + temp_instance + ':' + temp_file_path + ' -q \n')
                if is_zip:
                    command_arr.append('call gcloud compute ssh ' + temp_instance + ' --zone ' + zone + ' ' + ' --command "cd ' + temp_file_path + '; sudo unzip ' + temp_file_path + projectname + '" -q \n')
    ...
    def run_program(self, projectname, starter, args = ''):
        ...
        #Kill the processes after finished
        for p in processes:
            p.kill()
